...
geweke_results_plt_plt_sparse <- purrr::map(overfit_plt_setting_10_post_draws_UGLT, function(x){
  LaplacesDemon::Geweke.Diagnostic(x$T_stat)
})
ESS_results_plt_plt_sparse <- purrr::map(overfit_plt_setting_10_post_draws_UGLT, function(x) {
  LaplacesDemon::ESS(x$T_stat)
})
lengths_plt_plt_sparse <- purrr::map(overfit_plt_setting_10_post_draws_UGLT, function(x) {
  length(x$T_stat)
})
geweke_results_plt_uglt <- purrr::map(overfit_plt_setting_10_post_draws_plt_sparse, function(x){
  LaplacesDemon::Geweke.Diagnostic(x$T_stat)
})
ESS_results_plt_uglt <- purrr::map(overfit_plt_setting_10_post_draws_plt_sparse, function(x) {
  LaplacesDemon::ESS(x$T_stat)
})
lengths_plt_uglt <- purrr::map(overfit_plt_setting_10_post_draws_plt_sparse, function(x) {
  length(x$T_stat)
})
